...
A página a seguir explica como configurar o **Pipeline**. Para consultar mais informações conceituais sobre como o pipeline funciona, consulte a [visão geral do pipeline CI/CD](ci-cd-pipeline.md).
...
## Compreensão do fluxo {#understanding-the-flow}
...
>[!CAUTION]
>
>O pipeline não pode ser configurado até que o repositório Git tenha pelo menos uma ramificação e [Configuração do Programa](setting-up-program.md) esteja concluído.

Antes que você start implantar seu código, é necessário definir as configurações de pipeline a partir de [!UICONTROL Cloud Manager].
...
Depois de configurar seu programa usando a interface do usuário [!UICONTROL Cloud Manager], você estará pronto para configurar seu pipeline.
...
1. A tela **Setup Pipeline** é exibida.

   O assistente de três etapas permite configurar seu ambiente **Branch**, **Ambientes** e **Teste**.
Selecione sua ramificação Git e clique em **Próximo**.
...
1. Acesse a guia **Ambientes** para selecionar as opções **Stage** e **Production**.
...
>[!NOTE]
>
>Se a opção **Programado** estiver selecionada, você poderá programar sua implantação de produção para o pipeline **depois de** a implantação do palco (e **Usar Aprovação de GoLive**, se isso tiver sido ativado) para aguardar a definição de uma programação. O usuário também pode optar por executar a implantação de produção imediatamente.
>
>Consulte [**Implantar seu Código**](deploying-code.md) para definir o cronograma de implantação ou executar a produção imediatamente.

![](assets/configure-pipeline3.png)

>[!NOTE]
>
>A opção **Usar a Superintendência CSE** não está disponível para todos os clientes.

**Aprovar após a implantação do estágio**

Há uma etapa opcional **Aprovar após a implantação do palco** que pode ser configurada no pipeline de produção.
Isso está ativado em uma nova opção na tela **Edição do pipeline**:
...
1. Insira o caminho, selecione a ação de **Type** e clique em **Adicionar**. É possível especificar até 100 caminhos por ambiente. Depois de adicionar os caminhos, clique em **Aplicar**.
...
1. Acesse a guia **Teste** para definir seus critérios de teste para o seu programa.
   ...
   Você pode configurar *AEM Sites* e *AEM Assets* Teste de desempenho, dependendo dos produtos que você licenciou.
   ...
   * Cada uma das 3000 páginas no conjunto Novas páginas será acessada uma vez - ((200 * 0.5) / 3000) * 30 = 1
   ...
   Consulte [Teste de desempenho autenticado](#authenticated-performance-testing) para obter mais detalhes.
...
1. Clique em **Salvar** para concluir a configuração do processo de pipeline.
   ...
   >[!NOTE]
   >
   >Além disso, depois de configurar o pipeline, você ainda poderá editar as configurações do mesmo usando o bloco **Configurações do pipeline de produção** da interface do usuário [!UICONTROL Cloud Manager].
...
O nome de usuário e a senha são especificados como [Variáveis de pipeline](/help/using/build-environment-details.md#pipeline-variables) com os nomes `CM_PERF_TEST_BASIC_USERNAME` e `CM_PERF_TEST_BASIC_PASSWORD`.
...
Para definir essas variáveis usando a [CLI do Cloud Manager](https://github.com/adobe/aio-cli-plugin-cloudmanager), execute:
...
### Mecanismos de Não Produção e Qualidade de Código Somente do Gerenciador de Nuvem {#non-prod-video}
...
1. Clique em **Salvar** e o pipeline é mostrado no cartão na tela inicial com três ações:
   ...
   * **Build** - navega até a página de execução, a partir da qual o pipeline pode ser executado
   ...
   >[!NOTE]
   >
   >Enquanto o pipeline estiver em execução, a etapa atual será exibida e apenas a ação **Details** estará disponível.
