...
1. Esse subdiretório conterá um módulo Maven que cria o arquivo zip Dispatcher usando o Plug-in Maven Assembly. To start this, in the  directory, create a  file with this content, changing the parent reference, artifactId, and name as necessary.`dispatcher``pom.xml`
...
1. Now, create a subdirectory named  (as referenced in the assembly descriptor above on line 11) inside the dispatcher subdirectory to store the actual Apache and Dispatcher configurations. `src` Dentro desse `src` diretório, crie diretórios nomeados `conf`, `conf.d`, `conf.dispatcher.d`e `conf.modules.d`.
